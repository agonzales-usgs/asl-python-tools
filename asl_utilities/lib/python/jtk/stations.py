...
Python 2.5 is the minimum recommended version.
...
class Station:
    ...
    def run(self, action):
        try:
            ...
            self._log( "Station::run_update() caught exception: %s" % str(e), "error" )
    ...
    def ssh_disconnect(self):
        try:
            ...
            raise ExDisconnectFailed, "Disconnect Failed: %s" % e.__str__()
...
class Station680(Station):
    ...
    def build_summary(self, lines=None):
        ...
        ram_used = 0
        matches = reg_ram_used.findall( lines )
        if matches:
            ram_used = int( matches[0] )

        if ram_total and ram_used:
            percent_free = 100.0 - (float(ram_used) / float(ram_total) * 100.0)
...
class Station330(Station):
    def __init__(self, legacy=False, continuity_only=False, versions_only=False):
        Station.__init__(self)
    ...
    def transfer(self):
        for (dst, srcs) in self.transfer_list:
            ...
            if not len(source_files):
                continue
            command = "scp %s %s@%s:%s/." % ( ' '.join(source_files), self.username, self.address, dst)
